...
judge could see; on a vague one it LOSES a blind comparison 6 for 6. Pass it when you
know exactly what you are asking -- typically when a program composed the question --
and leave it off when a person phrased it. `--max-sources N` caps evidence gathering.
...
returns just the short answer when that is all you need.
